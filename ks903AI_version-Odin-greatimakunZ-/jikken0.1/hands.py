    ...
    while True:
        ...
        if face_results.multi_face_landmarks:
            for face_landmarks in face_results.multi_face_landmarks:
                ...
                debug_image = draw.overlay_Image(debug_image, face_brect)
        ...
        if results.multi_hand_landmarks is not None and mode != 9:
            ...
            if left_fg == 1 and right_fg == 1:
                ...
                if get_timestamp_int_from_time() - v.del_lock_tm > 100 and 2 <= len(v.input_letters):
                    v.input_letters = v.input_letters[:-2] + v.input_letters[-1]
                    v.del_lock_tm = get_timestamp_int_from_time() 
        # 手を認識していないときは入力文字を全削除 #
        else:
            if get_timestamp_int_from_time() - v.del_lock_tm > 300:
                v.input_letters = "" 
                v.del_lock_tm = get_timestamp_int_from_time()
...
def get_timestamp_int_from_time():
    # 1. time.time()でUNIXエポック(1970年1月1日)からの経過秒数を浮動小数点数で取得
    # 2. 1000を掛けて「ミリ秒」単位に変換
    # 3. int()で整数として返す
    return int(time.time() * 1000)

# 実行例
timestamp = get_timestamp_int_from_time()
print(f"取得したタイムスタンプ（経過ミリ秒）: {timestamp}")
